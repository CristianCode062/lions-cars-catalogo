...
const AutoCarousel = ({ images, interval = 3000 }: { images: string[], interval?: number }) => {
  ...
  useEffect(() => {
    console.log('🎠 AutoCarousel recibió imágenes:', images);
  }, [images]);
      ...
      <AnimatePresence mode="wait">
        <motion.img
          ...
          onError={(e) => {
            ...
            (e.target as HTMLImageElement).src = "/logos/autoefec.png";
          }}
          onLoad={() => {
...
import {
  ...
  Zap, BarChart3, Clock, ShieldCheck, ChevronRight, ArrowUpRight,
  ArrowDownRight, Bell, History, Target, Percent, Eye, Users, Award, Lock,
  Activity, Package, AlertTriangle, LineChart
} from 'lucide-react';
...
export default function SellerPortal({ stock, onAdd, onUpdate, onDelete, onBack }: SellerPortalProps & { onAdd: (car: Vehiculo) => void; onUpdate: (car: Vehiculo) => void; onDelete: (id: number) => void; }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#050505] via-[#0a0a0a] to-[#050505] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <motion.button
        ...
        onClick={onBack}
        className="absolute top-4 left-8 flex items-center gap-2 text-neutral-400 hover:text-white transition-colors group"
      >
...
const LionsEliteDashboard: React.FC<DashboardProps> = ({
  ...
  onBack,
  onLogout,
  // removed reloadStock from props
}) => {
          ...
          <AnimatePresence mode="wait">
            {view === 'overview' && (
              <motion.div key="overview" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="p-4 space-y-6">
                ...
                {/* Video hero eliminado por solicitud del usuario */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <StatCard
                    ...
                    value={`$${(stats.avgPrice / 1000000).toFixed(1)}`}
                    unit="M"
                    icon={DollarSign}
                    trend="up"
                    trendValue="8%"
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center justify-between mb-10">
                      ...
                      <div className="flex gap-2">
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className="px-4 py-2 bg-white/5 rounded-xl text-[10px] font-bold uppercase hover:bg-white/10 transition-colors border border-white/5"
                        >
                          Exportar
                        </motion.button>
                      </div>
                        ...
                        return (
                          ...
                          >
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    {hasDropped && (
                                      <motion.span
                                        initial={{ scale: 0 }}
                                        animate={{ scale: 1 }}
                                        className="flex items-center gap-1 text-[8px] bg-green-500/10 text-green-400 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-tighter"
                                      >
                            ...
                            <div className="h-1.5 w-full bg-neutral-900 rounded-full overflow-hidden">
                              <motion.div
                                initial={{ width: 0 }}
                                animate={{ width: `${Math.min(conversion * 5, 100)}%` }}
                                transition={{ delay: idx * 0.1 + 0.3, duration: 0.8 }}
                                className={`h-full rounded-full ${conversion > 10 ? 'bg-gradient-to-r from-yellow-600 to-yellow-400' : 'bg-neutral-700'}`}
                              />
                            </div>
                          </motion.div>
                        );
                  ...
                  <div className="bg-[#080808] border border-white/5 rounded-[2.5rem] p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
                    <motion.div
                      ...
                      className="absolute top-0 right-0 p-20 bg-blue-500/5 blur-[80px] rounded-full"
                    />
                    ...
                    <div className="mt-10 grid grid-cols-2 gap-4 w-full">
                      <motion.div
                        whileHover={{ scale: 1.05 }}
                        className="bg-neutral-900/50 p-4 rounded-3xl border border-white/5 hover:border-red-500/30 transition-all cursor-pointer"
                      >
  ...
  <VehicleForm
    ...
    onSubmit={(data) => {
      // SOLUCIÓN: Usar if/else explícito
      if (editingCar) {
        onUpdate(data);
      } else {
        onAdd(data);
...
// ===== INVENTORY VIEW (FIXED & FULL WIDTH) =====
...
const VehicleForm: React.FC<VehicleFormProps> = ({ car, onCancel, onSubmit }) => {
  ...
  marca: '', modelo: '', precio: 0, km: 0, ano: 2024,
  transmision: 'Automática', combustible: 'Gasolina', tipoVenta: 'Propio',
  estado: 'Disponible', imagen: '', imagenes: [], patente: '', color: '',
  vistas: 0, interesados: 0, diasStock: 0, comisionEstimada: 0,
  precioHistorial: [], vendedor: 'Admin Elite'
});
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!marcaValida || !modeloValido || !formData.precio || !formData.ano) {
  alert('Por favor completa los campos obligatorios: Marca, Modelo, Año y Precio.');
  console.warn('Submit bloqueado por datos inválidos:', { marca: formData.marca, modelo: formData.modelo, precio: formData.precio, ano: formData.ano });
  return;
}
...
console.log('📸 Imágenes:', vehiculoCompleto.imagenes);
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-5xl mx-auto pb-32">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-6">
          <button
            // removed reloadStock handler
            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs shadow border border-blue-700/30 transition-all"
            title="Recargar stock desde localStorage"
          >
            Recargar Stock
          </button>
          <motion.button
          ...
          <div>
            ...
            <p className="text-neutral-500 text-xs font-bold uppercase tracking-[0.2em] mt-1">Configuración técnica y comercial</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <FormSection title="Identidad del Vehículo" icon={Car} color="text-red-500">
            ...
            <div className="grid grid-cols-2 gap-6">
              <Field label="Año de Fab." type="number" value={formData.ano} onChange={(v) => setFormData({ ...formData, ano: parseInt(v) || 2024 })} />
              <Field label="Placa / Patente" value={formData.patente} onChange={(v) => setFormData({ ...formData, patente: v.toUpperCase() })} />
            </div>
          </FormSection>

          <FormSection title="Estrategia Comercial" icon={DollarSign} color="text-green-500">
            <div className="grid grid-cols-2 gap-6">
              <Field label="Precio de Lista" type="number" value={formData.precio} onChange={(v) => setFormData({ ...formData, precio: parseInt(v) || 0 })} />
              <Field label="Comisión Sugerida" type="number" value={comisionEstimada} onChange={() => { return; }} readOnly />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <SelectField
                label="Modalidad"
          ...
          <FormSection title="Ficha Técnica" icon={ShieldCheck} color="text-blue-500">
            <div className="grid grid-cols-2 gap-6">
              <Field label="Odómetro (KM)" type="number" value={formData.km} onChange={(v) => setFormData({ ...formData, km: parseInt(v) || 0 })} />
              <Field label="Color Exterior" value={formData.color} onChange={(v) => setFormData({ ...formData, color: v })} />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <SelectField
                label="Transmisión"
                value={formData.transmision}
                options={['Automática', 'Mecánica', 'PDK', 'Manual']}
                onChange={(v) => setFormData({ ...formData, transmision: v })}
              />
              <SelectField
                label="Combustible"
                value={formData.combustible}
                options={['Gasolina', 'Diesel', 'Híbrido', 'Eléctrico']}
                onChange={(v) => setFormData({ ...formData, combustible: v })}
              />
            </div>
          </FormSection>

          <FormSection title="Visual Assets" icon={ImageIcon} color="text-purple-500">
            <label className="text-[10px] font-black text-neutral-600 uppercase tracking-widest ml-1">Fotografía Principal</label>
            <div className="flex flex-col gap-2">
              <motion.label
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                className="cursor-pointer bg-gradient-to-r from-purple-700/30 to-red-500/20 border border-white/10 rounded-2xl px-5 py-4 text-sm text-white font-bold flex items-center gap-3 shadow-lg hover:from-red-500/30 hover:to-yellow-500/20 transition-all mb-2"
              >
  ...
  e.target.value = '';
              ...
              {formData.imagenes && formData.imagenes.length > 0 && (
                <motion.div layout className="flex flex-wrap gap-3 mb-2">
                  {formData.imagenes.map((img, idx) => (
                    ...
                    >
                      <img
                        src={img}
                        alt={`Miniatura ${idx+1}`}
                        className="w-20 h-14 object-cover rounded-xl border-2 border-white/10 shadow group-hover:shadow-lg group-hover:border-red-500 transition-all"
                      />
                      <motion.button
                        whileHover={{ scale: 1.2, rotate: 10 }}
                        whileTap={{ scale: 0.9, rotate: -10 }}
                        className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1 shadow-lg z-10 border-2 border-white/50 hover:bg-red-800 transition-all"
                        type="button"
                        title="Eliminar imagen"
                        onClick={() => {
                          setFormData(prev => {
                            const nuevas = (prev.imagenes || []).filter((_, i) => i !== idx);
                            return {
                              ...prev,
                              imagenes: nuevas,
                              imagen: nuevas[0] || ''
            ...
            <div className="aspect-[16/9] bg-neutral-900 border border-white/5 rounded-[2rem] overflow-hidden mt-2 group relative">
              {(formData.imagenes && formData.imagenes.length > 0) ? (
                <AutoCarousel images={formData.imagenes} interval={3500} />
              ) : formData.imagen ? (
                <motion.img
                  whileHover={{ scale: 1.1 }}
                  src={formData.imagen}
                  ...
                  onError={(e) => {
                    ...
                    console.error('No se pudo cargar la imagen:', formData.imagen);
                  }}
                />
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-neutral-700">
                  <ImageIcon size={48} strokeWidth={1} />
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] mt-4">Preview No Disponible</p>
                </div>
              )}
            </div>
          </FormSection>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-10">
          <motion.button
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="flex-1 bg-red-500 text-black font-black py-5 rounded-[2rem] shadow-2xl shadow-red-500/20 hover:scale-[1.02] active:scale-95 transition-all text-sm italic uppercase"
          >
            {car ? 'CONFIRMAR ACTUALIZACIÓN DE STOCK' : 'INGRESAR VEHÍCULO AL SISTEMA'}
          </motion.button>
          <motion.button
            type="button"
            onClick={onCancel}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="px-12 bg-neutral-900 border border-white/5 font-bold rounded-[2rem] text-neutral-500 hover:text-white transition-all text-sm"
          >
            DESCARTAR
          </motion.button>
        </div>
